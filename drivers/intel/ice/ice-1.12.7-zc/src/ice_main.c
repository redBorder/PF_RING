...
{
	struct ice_rx_ring *rx_ring = (struct ice_rx_ring*) data;
#ifdef ICE_ZC_IRQ
	int new_packets;
	...
	if (mode == 1 /* Enable interrupt */) {
		...
		if (!new_packets) {
			rx_ring->pfring_zc.interrupt_received = 0;

			if (!rx_ring->pfring_zc.interrupt_enabled) {
				/* Enabling interrupts on demand, this has been disabled with napi in ZC mode */
				ice_enable_interrupt(rx_ring->q_vector);

				rx_ring->pfring_zc.interrupt_enabled = 1;
			...
			rx_ring->pfring_zc.interrupt_received = 1;
		...
		rx_ring->pfring_zc.interrupt_enabled = 0;
...
int wake_up_pfring_zc_socket(struct ice_rx_ring *rx_ring)
{
	...
	if (atomic_read(&rx_ring->pfring_zc.queue_in_use)) {
		if (waitqueue_active(&rx_ring->pfring_zc.packet_waitqueue)) {
			if (ring_is_not_empty(rx_ring)) {
				rx_ring->pfring_zc.interrupt_received = 1;
				rx_ring->pfring_zc.interrupt_enabled = 0; /* napi disables them */
				wake_up_interruptible(&rx_ring->pfring_zc.packet_waitqueue);
				if (unlikely(enable_irq_debug))
...
{
	struct ice_rx_ring  *rx_ring = (struct ice_rx_ring *) rx_data;
	struct ice_tx_ring  *tx_ring = (struct ice_tx_ring *) tx_data;
	struct ice_vsi   *vsi;
	...
	if (device_in_use) { /* free all kernel buffers */
		...
		if ((n = atomic_dec_return(&adapter->pfring_zc.usage_counter)) == 0 /* last interface user */) {
			...
			ice_for_each_rxq(vsi, i) {
				...
				memset(rx_ring_i->desc, 0, rx_ring_i->pfring_zc.size);
...
{
	struct ice_rx_ring  *rx_ring = (struct ice_rx_ring *) rx_adapter;
	struct ice_vsi   *vsi;
	struct ice_pf    *adapter;
	struct ice_hw    *hw; 
	bool wr_main = true;
	...
	if (rx_ring == NULL) return -EINVAL; /* safety check */

	vsi = rx_ring->vsi;
	adapter = vsi->back; /* or use ice_netdev_to_pf(rx_ring->netdev); */
	hw = &adapter->hw;

	/* Patch to convert to system ticks with ICE_SRC_TMR_MODE_LOCKED */
	time_ns = ice_ptp_ns2ticks(adapter, time_ns);

	return ice_ptp_init_time(hw, time_ns, wr_main);
}
...
int adjust_time_callback(void *rx_adapter, int64_t offset_ns)
{
	...
	adj = offset_ns;
	if (adj >= 0)
		adj = (s32)ice_ptp_ns2ticks(adapter, adj);
	else
		adj = -((s32)ice_ptp_ns2ticks(adapter, -adj));

	return ice_ptp_adj_clock(hw, adj, true);
}

int get_tx_time_callback(void *tx_adapter, u_int64_t *time_ns)
{
	struct ice_tx_ring  *tx_ring = (struct ice_tx_ring *) tx_adapter;
	struct ice_vsi   *vsi;
	struct ice_pf    *adapter;
	struct ice_hw    *hw; 
	u8		 idx;
	...
	*time_ns = 0;
	...
	if (tx_ring == NULL) return -EINVAL; /* safety check */
	...
	rc = ice_read_phy_tstamp(hw, block, idx, &raw_tstamp);

	if (rc)
		return rc;

	/* Patch to convert from system ticks with ICE_SRC_TMR_MODE_LOCKED */
	//*time_ns = ice_ptp_ticks2ns(adapter, raw_tstamp);
	*time_ns = ice_ptp_extend_40b_ts(adapter, raw_tstamp);
...
static int ice_up_complete(struct ice_vsi *vsi)
{
	...
	if (vsi->netdev) {
		...
		ice_for_each_rxq(vsi, i) {
			...
			init_waitqueue_head(&rx_ring->pfring_zc.packet_waitqueue);
			...
			rx_info.descr_packet_memory_tot_len = rx_ring->pfring_zc.size;
			...
			tx_info.descr_packet_memory_tot_len = tx_ring->pfring_zc.size;
			...
			pf_ring_zc_dev_handler(add_device_mapping,
				...
				intel_ice,
				(char *) rx_ring->netdev->dev_addr,
				&rx_ring->pfring_zc.packet_waitqueue,
				&rx_ring->pfring_zc.interrupt_received,
				(void *) rx_ring,
				(void *) tx_ring);
		}
	}
