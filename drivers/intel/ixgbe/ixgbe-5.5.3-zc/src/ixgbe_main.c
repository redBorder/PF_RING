...
int ring_is_not_empty(struct ixgbe_ring *rx_ring) {
	...
	last_read = IXGBE_READ_REG(&adapter->hw, IXGBE_RDT(rx_ring->reg_idx));
...
int notify_function_ptr(void *rx_data, void *tx_data, u_int8_t device_in_use) 
{
	...
	if (!device_in_use) {
		if (rx_ring != NULL && rx_ring->desc == NULL) {
			printk("%s() failure: rx_ring->desc is NOT set\n", __FUNCTION__);
			return -1;
		}
		if (tx_ring != NULL && tx_ring->tx_buffer_info == NULL) {
...
#ifdef HAVE_PF_RING
		if (adapter->hw.silicom.has_hw_ts_card) {
...
#endif
				adapter->hw.silicom.last_9_bytes_ts_sec = signature->sec;
			} else if (signature->status == 0xC2) {
				ts_len = 5;
#if(LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,22))
				skb->tstamp = ktime_set((long)ntohl(adapter->hw.silicom.last_9_bytes_ts_sec), (long)ntohl(signature->nsec));
#else
				skb->tstamp.off_sec = ntohl(adapter->hw.silicom.last_9_bytes_ts_sec);
				skb->tstamp.off_usec = ntohl(signature->nsec) / 1000;
#endif
			} else {
				ts_len = 0;
			...
			const u8 hdr_len)
{
	...
	if (atomic_read(&adapter->pfring_zc.usage_counter) > 0)
		return -1; /* We don't allow apps to send data */	
#endif
