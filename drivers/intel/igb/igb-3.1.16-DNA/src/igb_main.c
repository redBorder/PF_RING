...
#ifdef ENABLE_DNA
	{
	  ...
	  if(hook) {
		...
		rx_ring->dna.rx_tx.rx.packet_memory[i] = 0;
	      ...
	      printk("[DNA] ring_dna_device_handler=%p\n", hook->ring_dna_device_handler);
	      ...
	      printk("%s(): [%s@%d] WARNING Not allocated RX DMA memory\n",
		     __FUNCTION__, tx_ring->netdev->name, tx_ring->queue_index);
	    } /* if */
	  } /* if */
	} /* dna */
...
#ifdef ENABLE_DNA
	return(0); /* We don't allow apps to send data */	
#endif
...
#ifdef ENABLE_DNA
	dna_igb_clean_rx_irq(q_vector, rx_ring, budget);
	return;
#endif
