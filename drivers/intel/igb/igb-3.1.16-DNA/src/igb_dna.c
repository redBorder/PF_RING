...
void dna_igb_alloc_rx_buffers(struct igb_ring *rx_ring, struct pfring_hooks *hook) {
  ...
  rx_ring->dna.packet_slot_len  = ALIGN(rx_ring->size, cache_line_size);
...
static int dna_igb_rx_dump(struct igb_ring *rx_ring) {
  int j, found=0;

  for(j=0; j<rx_ring->count; j++) {
    union e1000_adv_rx_desc *rx_desc = IGB_RX_DESC(rx_ring, j);
